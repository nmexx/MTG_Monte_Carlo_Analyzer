...
37. **Oracle-text fallback detection for treasure & draw cards** --------DONE
    - **`cardProcessors.js`** — two new exported helpers inserted after `hasManaTapAbility`:
      ...
      - `detectDrawFromOracle(oracleText, typeLine)` — returns a `CARD_DRAW_DATA`-shaped object (or `null`) by matching the `draw a/N card(s)` pattern. Excludes symmetrical effects ("each player draws", "each opponent draws", "target player draws", "they draw"). Infers `cardType` from the type line; sets `triggerType` to `'upkeep'` for recurring effects.
    ...
    - **Tests** — 19 new tests in `cardProcessors.test.js`: 8 for `detectTreasureFromOracle`, 9 for `detectDrawFromOracle`, 2 new `processCardData` routing tests for the oracle fallback; total suite: 543 tests.
